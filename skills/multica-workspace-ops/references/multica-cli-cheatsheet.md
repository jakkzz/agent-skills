# Multica CLI Cheatsheet for the workspace
...
W="<the workspace workspace ID from live output>"
```
...
multica --workspace-id "$W" issue create \
  --title "Issue title" \
  --description-file /tmp/multica-issue.md \
  --project "PROJECT_ID" \
  --priority high \
  --output json
```
