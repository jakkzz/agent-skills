...
const approvalActor = process.env.ACADEMIC_BOOK_APPROVER?.trim() || process.env.USER || "approver";
const approvalNotes =
  process.env.ACADEMIC_BOOK_APPROVAL_NOTES?.trim() ||
  "Approved through interactive Pi confirmation after review of the displayed artifact and SHA-256.";
...
function isMissingBookState(error: unknown): boolean {
...
async function refreshStatus(pi: ExtensionAPI, ctx: any, cwd = ctx.cwd) {
  ...
  try {
    ...
    if (!ctx.hasUI) return;
    if (isMissingBookState(error)) {
      ctx.ui.setStatus("academic-book", undefined);
      return;
    }
    ctx.ui.setStatus("academic-book", ctx.ui.theme.fg("warning", "Book state error"));
  }
}
...
export default function academicBookStudio(pi: ExtensionAPI) {
    ...
    handler: async (args, ctx) => {
      ...
      try {
        ...
        const payload = await runBookctl(
          pi,
          ctx.cwd,
          ["approve", "--chapter", chapter, "--gate", gate, "--approved-by", approvalActor, "--notes", approvalNotes],
          ctx.signal,
        );
  ...
  pi.registerCommand("evidence-approve", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!relation) return;
      const reviewedBy = await ctx.ui.input("Reviewer name", process.env.USER || "reviewer");
